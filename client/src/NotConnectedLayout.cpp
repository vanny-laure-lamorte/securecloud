...
NotConnectedLayout::NotConnectedLayout(ClientService *service, QWidget *parent, TranslationManager *tm)
    : QWidget(parent), service_(service), translationManager_(tm)
{
    ...
    connect(translationManager_, &TranslationManager::languageChanged, this, [=](const QString &lang)
            {
        clearPrincipalPage();
        setPrincipalPage();
     });
...
void NotConnectedLayout::wireBody(QWidget *bodyWidget)
{
    ...
    {
        connect(loginPage, &Login::loginRequested, this, [this](const QString &email, const QString &password)
                {
            if (!service_)
                return;
    ...
    {
        connect(homePage, &Home::logoutRequested, this, [this]()
                {
            if (!service_)
                return;
...
void NotConnectedLayout::setPrincipalPage()
{
    // Header
    ...
    // Body
    body = new Login(this, translationManager_);
...
void NotConnectedLayout::clearPrincipalPage(){
        mainLayout->removeWidget(header);
        mainLayout->removeWidget(body);
        mainLayout->removeWidget(footer);
        header->deleteLater();
        body->deleteLater();
        footer->deleteLater();
}